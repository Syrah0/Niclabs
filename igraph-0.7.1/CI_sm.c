...
int init_CI(char *name, int rad, char *mode, int nodeComp, int pos){
	FILE *F, *G, *H;
	char filename[300];
	igraph_t graph, gaux;
...
/* GENERACION ARCHIVOS
	...
	sprintf(filename, "CI_times_graph%d.csv",pos);
	G = fopen(filename,"w"); // archivo que guardara los tiempo de ejecucion por iteracion

	sprintf(filename, "CI_iter_graph%d.csv",pos);
...
//	fprintf(stderr, "degree:%f nodes:%d\n", k,N);
...
/* GENERACION ARCHIVOS
		if(rem_nodes == ceil(total_nodes*remove)){
		    sprintf(filename,"grafo%d_CI_graph%d.edges",(total_nodes - (int)rem_nodes),pos); // nombre del archivo donde estaran los resultados
		    F = fopen(filename,"w");
...
/* GENERACION ARCHIVOS
	sprintf(filename, "TiempoEjecución_CI_graph%d.txt",pos);
	G = fopen(filename,"w");
...
/* GENERACION ARCHIVOS
	sprintf(filename, "grafoFinal_CI_graph%d.edges",pos);
	F = fopen(filename,"w");
