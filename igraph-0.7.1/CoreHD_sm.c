...
int init_CoreHD(char *name, int nodeComp, int pos){
	FILE *F, *G, *H;
	char filename[300];
	igraph_t graph, gaux;
...
/* GENERACION ARCHIVOS
	...
	sprintf(filename, "CoreHD_times_graph%d.csv",pos);

	G = fopen(filename,"w"); // archivo que guardara los tiempos de ejecucion por iteracion

	sprintf(filename, "CoreHD_iter_graph%d.csv",pos);
...
/*	GENERACION ARCHIVOS
		if(rem_nodes == ceil(total_nodes*remove)){
			sprintf(filename,"grafo%d_CoreHD_graph%d.edges",(total_nodes - (int)rem_nodes), pos); // nombre del archivo donde estaran los resultados
		    F = fopen(filename,"w");
...
/* DESCOMENTAR SI SE USA G Y/O H
	...
	fclose(H);
/* GENERACION ARCHIVOS
	sprintf(filename, "TiempoEjecución_CoreHD_graph%d.txt",pos);
...
/* GENERACION ARCHIVOS	
	sprintf(filename, "grafoFinal_CoreHD_graph%d.edges",pos);
